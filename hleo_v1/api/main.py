...
def assistant_chat(
    ...
    if sc and sc.articles:
        ...
        search_block = (
            ...
            + "\n\n"
            "══ RESPONSE STRATEGY ══\n"
            "You are a clinical expert. The user has asked a question. "
            "Read the question carefully and answer it directly.\n\n"
            "STEP 1 — Identify what the user is actually asking:\n"
            "  • A factual question about a drug/condition/mechanism → answer the fact first.\n"
            "  • A request to compare treatments → identify the comparison studies first.\n"
            "  • A request to summarise evidence → open with the overall conclusion.\n"
            "  • A question about a specific side-effect → state whether it occurs and how often, first.\n\n"
            "STEP 2 — Build the response in this order (adapt section names to the question):\n\n"
            "## [Direct Answer — title that reflects the specific question]\n"
            "2–5 concise sentences that directly answer what was asked. "
            "Ground this in the retrieved articles. "
            "NEVER open with: 'Search performed', 'X articles retrieved', "
            "'Based on the retrieved articles', or any description of the search itself. "
            "The first word of the response must address the user's question.\n\n"
            "## 🔬 [Evidence — title that reflects the supporting studies]\n"
            "Summarise ONLY the articles that are relevant to the question above. "
            "Cite each finding in brackets, e.g. '[PubMed – Article 2]'. "
            "Skip articles that do not contribute to answering the question. "
            "Do NOT paraphrase from general knowledge.\n\n"
            "## 🩺 [Clinical Interpretation — concise synthesis]\n"
            "3–5 sentences synthesising the evidence: highlight agreement, disagreement, "
            "strength of evidence, and key limitations. "
            "Do not repeat findings already stated above.\n\n"
            "## ℹ️ Additional Medical Knowledge (ONLY if needed)\n"
            "Include this section ONLY when the retrieved articles leave a clinically important gap. "
            "If included, open with exactly: "
            "'The following information is not derived from the retrieved studies "
            "but from general medical knowledge:' "
            "If the articles fully answer the question, OMIT this section entirely.\n\n"
            "ABSOLUTE RULES:\n"
            "- The response MUST open with a direct answer to the question — never with search metadata.\n"
            "- Every claim in the Evidence and Clinical Interpretation sections must trace to a listed article.\n"
            "- Do not produce a response that could have been written without the retrieved articles.\n"
            "- Adapt section headings to reflect the actual question; do not use generic labels.\n"
            "══════════════════════════════════════════\n"
        )
    elif sc:
